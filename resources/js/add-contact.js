    ...
    delay = setTimeout(() => {
        findContacts(contactModalSearch.value)
            ...
            .then(resp => {
                ...
                if (resp.length === 0) {
                    if (contactModalSearch.value) {
                        let notFoundContainer = document.createElement('div');
                        notFoundContainer.innerHTML = 'Новых контактов по введенным данным не найдено'
                        notFoundContainer.classList.value = 'flex justify-center items-center text-emerald-600 font-16';
...
export function createContactBlock(contact) {
